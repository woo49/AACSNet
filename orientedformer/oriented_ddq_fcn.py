...
from mmdet.models.task_modules.prior_generators.point_generator import \
    MlvlPointGenerator
from .angle_aware_feature_alignment import AngleAwareFeatureAlignment
from .cross_level_synergy_module import CrossLevelSynergyModule
...
class OrientedDDQFCN(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.strides = strides  # Store strides for use in _init_layers
        super().__init__(*args,
                         strides=strides,
        ...
        # AAFAM and CLSM configurations
        self.aafam_cfg = aafam_cfg
        self.clsm_cfg = clsm_cfg
        ...
        # Initialize AAFAM and CLSM modules after prior_generator is created
        if self.aafam_cfg is not None:
            aafam_cfg = self.aafam_cfg.copy()
            if 'type' not in aafam_cfg:
                aafam_cfg['type'] = 'AngleAwareFeatureAlignment'
            # Update with required parameters (these override user config if provided)
            aafam_cfg['in_channels'] = self.feat_channels
            aafam_cfg['embed_dim'] = self.feat_channels
            aafam_cfg['angle_version'] = self.angle_version
            self.aafam = MODELS.build(aafam_cfg)
        else:
            self.aafam = None
        
        # Initialize CLSM module if configured
        if self.clsm_cfg is not None:
            clsm_cfg = self.clsm_cfg.copy()
            if 'type' not in clsm_cfg:
                clsm_cfg['type'] = 'CrossLevelSynergyModule'
            num_levels = len(self.prior_generator.strides)
            # Update with required parameters (these override user config if provided)
            clsm_cfg['in_channels'] = self.feat_channels
            clsm_cfg['num_scales'] = num_levels
            self.clsm = MODELS.build(clsm_cfg)
        else:
            self.clsm = None
    ...
    def forward(self, inputs: list, **kwargs):
        ...
        Returns:
            ...
            aux_results (dict[list]): 'cls_scores_list' (list[Tensor]): (bs, num_class, h, w),
                ...
                'reg_feats' (list[Tensor]): (bs, c, h, w).
        '''
        # Apply CLSM for cross-level feature enhancement if configured
        if self.clsm is not None:
            inputs = self.clsm(inputs)
        ...
        # Apply AAFAM for angle-aware feature alignment if configured
        if self.aafam is not None:
            enhanced_cls_feats = []
            enhanced_reg_feats = []
            for cls_feat, reg_feat in zip(cls_feats, reg_feats):
                # Apply AAFAM to both cls and reg features
                enhanced_cls = self.aafam(cls_feat)
                enhanced_reg = self.aafam(reg_feat)
                enhanced_cls_feats.append(enhanced_cls)
                enhanced_reg_feats.append(enhanced_reg)
            cls_feats = enhanced_cls_feats
            reg_feats = enhanced_reg_feats
