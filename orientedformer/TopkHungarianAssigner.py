...
class TopkHungarianAssigner(TaskAlignedAssigner):
    ...
    def assign(self,
        ...
        # Fix invalid values (NaN, Inf) early to prevent propagation
        all_cost = torch.where(torch.isfinite(all_cost), 
                               all_cost, 
                               torch.tensor(1e10, device=all_cost.device, dtype=all_cost.dtype))
        ...
        if num_gt > 0:
            ...
            # Fix invalid values (NaN, Inf) in cost matrix before linear_sum_assignment
            cost_np = repeat_select_cost.detach().cpu().numpy()
            # Replace NaN and Inf with a large finite value (1e10) to avoid invalid entries
            if not np.isfinite(cost_np).all():
                cost_np = np.nan_to_num(cost_np, nan=1e10, posinf=1e10, neginf=1e10)
